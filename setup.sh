...
installWarp(){
	...
	if [[ "${release}" == "debian" ]]; then
		...
		${installType} yum-utils 
		sudo rpm -ivh http://pkg.cloudflareclient.com/cloudflare-release-el${centosVersion}.rpm 
	fi

	echoContent green " ---> Install WARP"
	${installType} cloudflare-warp 
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		if wget --help | grep -q show-progress; then
			...
			wget -c -P /etc/v2ray-agent/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip" 
		fi

		unzip -o /etc/v2ray-agent/v2ray/${v2rayCoreCPUVendor}.zip -d /etc/v2ray-agent/v2ray 
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		if wget --help | grep -q show-progress; then
			...
			wget -c -P /etc/v2ray-agent/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip" 
		fi

		unzip -o /etc/v2ray-agent/xray/${xrayCoreCPUVendor}.zip -d /etc/v2ray-agent/xray 
...
installTrojanGo() {
	...
	if ! ls /etc/v2ray-agent/trojan/ | grep -q trojan-go; then
		...
		if wget --help | grep -q show-progress; then
			...
			wget -c -P /etc/v2ray-agent/trojan/ "https://github.com/p4gefau1t/trojan-go/releases/download/${version}/${trojanGoCPUVendor}.zip" 
		fi
		unzip -o /etc/v2ray-agent/trojan/${trojanGoCPUVendor}.zip -d /etc/v2ray-agent/trojan 
...
{
  "log": {
    "access": "/etc/v2ray-agent/xray/access.log",
...
#		rm -rf /usr/share/nginx/html
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" 
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/mack-a/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" 
		fi

		unzip -o "/usr/share/nginx/html${selectInstallNginxBlogType}.zip" -d /usr/share/nginx/html 
...
# Firewall
handleFirewall(){
	if systemctl status ufw 2|grep -q "active (exited)" && [[ "$1" == "stop" ]]; then
		systemctl stop ufw 
	...
	if systemctl status firewalld 2|grep -q "active (running)" && [[ "$1" == "stop" ]]; then
